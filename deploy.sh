...
kustomize edit set image devops-service=$IMAGE_NAME || { echo "Failed to edit image with kustomize"; exit 1; }
kustomize build . | kubectl apply -f - || { echo "Failed to build and apply kustomize"; exit 1; }

# Wait for deployment rollout to complete
kubectl rollout status deployment/devops-service -n $NAMESPACE || { echo "Failed to get rollout status for devops-service"; exit 1; }
